...
// Main execution
const main = async () => {
  // 1. Fetch GitHub stats and generate profile card SVGs
  ...
  // 2. Refresh profile image URLs so GitHub does not serve stale SVGs.
  const timestamp = new Date().getTime();

  // 3. Read template and write to README.md
  let template = fs.readFileSync("template.md").toString();
  template = template.replaceAll("{CACHE_BUSTER}", timestamp);
